...
def _sh(*args: str, cwd: Path | None = None) -> str:
    try:
        ...
        raise ReleaseError(" ".join(args)) from None
...
def project_deps(path: Path = PYPROJECT) -> tuple[str, ...]:
    return tuple(glom(_toml(path.read_text(encoding="utf-8")), "project.dependencies", default=()))
...
def _smoke(run: Runner, *installs: Sequence[str]) -> str:
    with tempfile.TemporaryDirectory(prefix="smoke-") as temporary:
        venv_dir = Path(temporary) / "venv"
        python = str(_venv_python(venv_dir))
        run((sys.executable, "-m", "venv", str(venv_dir)))
        for install in installs:
            run((python, "-m", "pip", "install", *install))
        return run((python, "-I", str(SMOKE)))


def smoke_install(spec: str, *, expected: str | None = None, runner: Runner | None = None) -> str:
    candidate = Path(spec)
    installed = _smoke(runner or run_process, ("--", str(candidate.resolve()) if candidate.exists() else spec))
...
def smoke_index(target: str, *, runner: Runner | None = None) -> str:
    ...
    installed = _smoke(
        runner or run_process,
        ("--no-cache-dir", "--index-url", INDEXES["pypi"][1], *project_deps()),
        ("--no-deps", "--no-cache-dir", "--index-url", INDEXES[target][1], f"{name}=={version}"),
    )
...
def dispatch(argv: Sequence[str] = sys.argv) -> str:
    ...
    match raw:
        ...
        case {"check-version": True, "<target>": target}:
            ...
            return f"ok {version} > {len(published)} on {_s(target)}"
