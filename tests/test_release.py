...
def test_check_version_requires_strictly_greater(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("scripts.release.read_project_metadata", lambda _: ("sample", Version("1.0")))
    monkeypatch.setattr("scripts.release.fetch_published_versions", lambda *_: (Version("0.9"), Version("1.0.0")))
...
def test_main_check_version_reports_available_package(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert stdout.getvalue() == "ok 2.0 > 1 on pypi\n"
...
def test_smoke_install_uses_clean_venv(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("scripts.release.tempfile.TemporaryDirectory", TemporaryDirectory)
    assert smoke_install(str(wheel), runner=runner) == "1.0"
    python = str(tmp_path / "temporary" / "venv" / "bin" / "python")
    assert [list(command) for command in commands] == [
        [sys.executable, "-m", "venv", str(tmp_path / "temporary" / "venv")],
        [python, "-m", "pip", "install", "--", str(wheel.resolve())],
        [python, "-I", str(SMOKE)],
    ]


def test_smoke_install_expected_mismatch(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_smoke_index_installs_from_target(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    commands: list[Sequence[str]] = []
    ...
    assert any("test.pypi.org" in part for command in commands for part in command)
