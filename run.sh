...
# .uv-version pins the uv binary for GHA and Docker; host uv uses required-version.
...
readonly ROOT_DIR
readonly VENV_BIN="$ROOT_DIR/.venv/bin"
DEFAULT_PYTHON="$(tr -d '[:space:]' < "$ROOT_DIR/.python-version")"
readonly DEFAULT_PYTHON
UV_PIN="$(tr -d '[:space:]' < "$ROOT_DIR/.uv-version")"
readonly UV_PIN
...
pkg:bump(){
  ...
  uv version "$1" || return $?
  printf 'commit the staged version bump with subject: v%s\n' "$(uv version --short)"
}
...
# @describe Require local version to exceed every version on an index
...
# @env UV_VERSION             Override .uv-version pin
docker:build(){
  ...
  docker build \
    ...
    "${target_arg[@]}" \
    -t "$IMAGE_NAME:$version" "$@" || return $?
  echo "$IMAGE_NAME:$version"
}

# @describe Build prod image + ./run.sh tests. Host at /mnt/app for later export
# @alias docker:tests
...
docker:test(){
  ...
  docker run $(docker:_flags) -v "$ROOT_DIR":/mnt/app "$img" tests "$@" || return $?
}
...
docker:shell(){
  local img extra=()
  img="$(docker:build dev)" || return $?
  [ -t 0 ] && [ -t 1 ] && extra=(-it)
  # shellcheck disable=SC2046
  docker run $(docker:_flags) "${extra[@]}" -v "$ROOT_DIR":/app "$img" "$@" || return $?
}
...
_workflow_remote(){
  ...
  [ "$run_id" != 0 ] && [ "$run_id" != "$before" ] || { echo "could not find dispatched $workflow run" >&2; return 1; }
...
_publish_local(){
  local -; set +x
  local target="$1" publish_url check_url version tag
  _need_venv || return $?
  _require_clean || return $?
  ...
  release:_check-version "$target" || return $?
...
if ! (return 0 2>/dev/null); then
  set -uo pipefail
  exec 2> >(grep -vF -e 'local -' -e 'set +x' >&2)
  PS4=$__PS4
  if [ $# -eq 0 ]; then
    main; exit $?
